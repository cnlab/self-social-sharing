...
if (!file.exists("models/sca_output_12.RDS")) {
  ...
  # divid df_3 into two subsets to reduce output file size
  subsetList <- function(myList, elementNames) {
      lapply(elementNames, FUN=function(x) myList[[x]])
  }
             
  df_3a = subsetList(df_3, 1:4)
  df_3b = subsetList(df_3, 5:9)
}
```

# run SCA {.tabset}
## run models
Run SCAs or load saved output from files.
...
if (!file.exists("models/sca_output_12.RDS")) {
  ...
  # run SCA
  output_12 = furrr::future_map_dfr(df_12, ~ run_spec(models, .x,
                                                       conf.level = conf_level,
                                                       keep.results = FALSE) %>%
                          mutate(subsets = unique(.x$filter)))
  # save output
  saveRDS(output_12, "models/sca_output_12.RDS")

} else {
  
  #load output
  output_12 = readRDS("models/sca_output_12.RDS")
}

# df_3a
if (!file.exists("models/sca_output_3a.RDS")) {
  # setup parallelization with furrr
  plan(multisession, workers = 10)
  
  # run SCA
  output_3a = furrr::future_map_dfr(df_3a, ~ run_spec(models, .x,
                                                       conf.level = conf_level,
                                                       keep.results = FALSE) %>%
                          mutate(subsets = unique(.x$filter)))
  # save output
  saveRDS(output_3a, "models/sca_output_3a.RDS")

} else {
  # load output
  output_3a = readRDS("models/sca_output_3a.RDS")
}

# df_3b
if (!file.exists("models/sca_output_3b.RDS")) {
  # setup parallelization with furrr
  plan(multisession, workers = 10)
  
  # run SCA
  output_3b = furrr::future_map_dfr(df_3b, ~ run_spec(models, .x,
                                                       conf.level = conf_level,
                                                       keep.results = FALSE) %>%
                          mutate(subsets = unique(.x$filter)))
  
  # save output
  saveRDS(output_3b, "models/sca_output_3b.RDS")

} else {
  # load output
  output_3b = readRDS("models/sca_output_3b.RDS")
}
```

## tidy output
```{r}
output1 = bind_rows(output_12, output_3a, output_3b) %>%
