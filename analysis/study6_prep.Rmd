...
This script tidies and prepares the cleaned data from Study 6.
...
pacman::p_load(tidyverse, tidyText, lmerTest)
```
...
articles = read.csv("../stimuli/study6_articles.csv", stringsAsFactors = FALSE)
...
comments = data_raw %>%
  ...
  mutate(cond = gsub("other", "social", cond)) %>%
...
data_mod_diss = messages %>%
  ...
  select(study, group, SID, item, article_cond, content, trial, sharing_type, msg_share, contains("rel_"))

```

# tidy demographic data
```{r}
demo = read.csv("../data/study6_clean_long_demo.csv", stringsAsFactors = FALSE) %>%
...
# write csvs
```{r}
write.csv(data_mod_diss, "../data/study6.csv", row.names = FALSE)
write.csv(demo, "../data/study6_demo.csv", row.names = FALSE)
write.csv(comments, "../data/study6_comments.csv", row.names = FALSE)
```
