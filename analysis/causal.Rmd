...
## define aesthetics
```{r}
palette_condition = c("#ffb800", "#E64626", "#1985A1")
...
study6 = read.csv("../data/study6.csv", stringsAsFactors = FALSE) %>%
  mutate(sharing_type = ifelse(sharing_type == 0, -.5, .5))
```
...
predicted_h3_h4 = ggeffects::ggpredict(mod_h3_h4, c("article_cond", "sharing_type")) %>%
              data.frame() %>%
  mutate(group = ifelse(group == "-0.5", "broadcast sharing", "narrowcast sharing"))
...
data_med_self = study6 %>%
  ...
  mutate(article_cond = ifelse(article_cond == "self", 1, 0),
         sharing_type = ifelse(sharing_type == -.5, "broadcast", "narrowcast"),
